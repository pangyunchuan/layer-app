...
export default abstract class LoadingRequest extends BaseRequest {
     ...
     */
    reqOne<M extends RequestModel<{}>, MD = Required<M["_dataType"]>>(Model: new () => M, call?: (inst: M & MD) => void): Promise<M & MD>;
    /**
     * 请求并返回单模型和其他数据
     * @param Model
     ...
     * @param call
     */
    reqOneOther<ApiData extends object, DK extends keyof ApiData, M extends RequestModel<any>, MD = Required<M["_dataType"]>>(Model: new () => M, dataKey: DK, call?: (inst: M & MD) => void): Promise<Omit<ApiData, DK> & {
        model: M & MD;
    }>;
    /**
     * 请求并返回模型数据
     ...
     * @param call
     */
    reqMany<M extends RequestModel<{}>, MD = Required<M["_dataType"]>>(Model: new () => M, call?: (inst: M & MD) => void): Promise<(M & MD)[]>;
    /**
     * 返回模型数组和其他字段
     * @param Model
     * @param dataKey  模型数据所在字段
     * @param call
     */
    reqManyOther<ApiData extends object, DK extends keyof ApiData, M extends RequestModel<{}>, MD = Required<M["_dataType"]>>(Model: new () => M, dataKey: DK, call?: (inst: M & MD) => void): Promise<Omit<ApiData, DK> & {
        models: (M & MD)[];
    }>;
}
