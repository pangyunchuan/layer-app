...
export default abstract class LoadingRequest extends BaseRequest {
    ...
    reqOne<M extends RequestModel<{}>, MD = Required<M["_dataType"]>>(
        Model: new() => M, call?: (inst: M & MD) => void
    ) {
    ...
    reqOneOther<ApiData extends object,
        ...
        Model: new() => M, dataKey: DK, call?: (inst: M & MD) => void
    ) {
        return this.request().then(res => {
     ...
     * @param call
     */
    reqMany<M extends RequestModel<{}>, MD = Required<M["_dataType"]>>(Model: new() => M, call?: (inst: M & MD) => void) {
        return this.request().then(res => {
            let models = [];
            for (const da of res) {
